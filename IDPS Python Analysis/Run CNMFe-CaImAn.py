...
data_dir = os.path.join('/path/to/dataset',
                        'IDPS_data file')

# Pick out a movie from a particular session.
mov_file = os.path.join(data_dir, 'movie.isxd')
...
pp_mov_file = os.path.join(data_dir, 'movie.isxd')
...
cellset_file = os.path.join(data_dir, 'movie-CNMFE.isxd')
events_file = os.path.join(data_dir, 'movie-CNMFE-ED.isxd')
